...
class BarcodeScannerUIView: UIView, AVCaptureMetadataOutputObjectsDelegate {
    ...
    func metadataOutput(_ output: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
        // Checks the hasDetected property
        if let metadataObject = metadataObjects.first,
           let readableObject = metadataObject as? AVMetadataMachineReadableCodeObject,
           let stringValue = readableObject.stringValue,
           !hasDetected { // Check here if a barcode has already been detected

            hasDetected = true // Mark that a barcode has been detected
            DispatchQueue.main.async {
                self.onDetected?(stringValue)
            }
        }
        
//        // Continuous scanning if barcode is available
//        if let metadataObject = metadataObjects.first,
//           let readableObject = metadataObject as? AVMetadataMachineReadableCodeObject,
//           let stringValue = readableObject.stringValue {
//            DispatchQueue.main.async {
